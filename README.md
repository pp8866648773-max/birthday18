...
<head>
  ...
  <style>
    ...
    html,body{height:100%; margin:0; font-family: 'Segoe UI', Roboto, Arial, sans-serif; background: radial-gradient(circle at 10% 10%, #2b1a3a 0%, var(--bg) 40%); color:white; -webkit-font-smoothing:antialiased;}
    .wrap{
      min-height:100%;
      display:flex;
      align-items:center;
    ...
    .card{
      ...
      border-radius:18px;
      box-shadow: 0 10px 30px rgba(2,2,6,0.6);
      overflow:hidden;
      position:relative;
      padding:24px;
...
<body>
  ...
  <!-- optional audio (replace src with your file) -->
  <!-- <audio id="bgm" src="happy-song.mp3" loop></audio> -->
  ...
  <script>
    ...
    // surprise button: small popup with confetti-like hearts (simple)
    function surprise(){
      for(let i=0;i<20;i++){
    ...
    // small friendly fallback: if photo.jpg missing, replace with placeholder
